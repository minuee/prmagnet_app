      ...
      if ( element.sample_list != null ) {
        element.sample_list.forEach(function(element2,index2){         
          if ( element2.sample_no ) {
            ...
            if (element2.pickup_dt != null &&  mUtils.dateToDate(element2.pickup_dt) != mUtils.convertUnixToDate(returning_date) ) {
              changeReturnDate = mUtils.convertDateToUnix(element2.pickup_dt);
            }
          }
        })
      }
          ...
          { !mUtils.isEmpty(this.state.setChangeReturnDate) &&
            <View style={styles.middleWrapper}>
              ...
              <Text style={styles.middleDescRedText}>
                {mUtils.getShowDate(this.state.setChangeReturnDate)} *일부반납(발송)일이 변경되었습니다.
              </Text>              
            </View>
          }
          <View style={styles.middleWrapper}>
                    ...
                    {_.map(samples, (subItem, subIndex) => {
                      ...
                      return (
                        <LinkSheetUnit
                          key={`${subItem.sample_no}${subIndex}`}
                          readOnly={false}
                          checked={checkedList.includes(subItem.sample_no) || subItem.pickup_yn }
                          name={fromName}
                          phone={fromPhone}
                          unitType={'to'}
                          viewType={'sendout'}
