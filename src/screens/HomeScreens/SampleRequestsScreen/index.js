  ...
  postSRRequestSend = async () => {
    ...
    try {
      const response = await API.postSRRequestSend({
        ...
        release_dt: is_media ? null : String(Math.floor(Number(rshDate.timestamp) / 1000)),
        release_end_dt: is_media ? null : String(Math.floor(Number(rshEndDate.timestamp) / 1000)),
        begin_dt: startTime,
        end_dt: endTime,
