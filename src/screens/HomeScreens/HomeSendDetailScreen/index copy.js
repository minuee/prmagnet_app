...
class HomeDetailScreen extends PureComponent {
  ...
  getHomeTR = async (nextpage = 1,type) => {
    ...
    if ( page * limit <= total_count ) {
      try {
        ...
        console.log('getHomeTR>>>122', error)
      }
    }else{
  ...
  renderPickupItem = async({item}) => {
    //console.log('rederrederreder',item);
    const {type, title} = this.props.route.params;
    const userType = mConst.getUserType();
    const subItem = item.each_list[0];
    
    ///console.log('subItem',selectedIndex,selectedIndex2,type,subItem)
    if (  type === 'pickups' ) {
      let showroomData = [];
      let reqNoData = [];
    ...
    return (
      <>
        <SafeAreaView style={styles.container}>
          ...
          {
            ( mConst.getUserType() === 'B' && type == "sendout" ) && (
              <View
                style={{
                  ...styles.layout1,
